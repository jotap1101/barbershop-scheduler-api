...
def get_dashboard_overview() -> Dict:
    ...
    active_barbers = User.objects.filter(
...
def get_barbershop_analytics(barbershop_id: str) -> Optional[Dict]:
    ...
    popular_barber = User.objects.filter(
        appointments__barbershop=barbershop,
        role=User.Role.BARBER
    ).annotate(
        appointment_count=Count('appointments')
    ).order_by('-appointment_count').first()
    ...
    return {
        ...
        'average_rating': round(average_rating, 2) if average_rating else Decimal('0.00'),
        'appointments_this_month': appointments_this_month,
        'revenue_this_month': revenue_this_month,
        'top_service': popular_service.name if popular_service else None,
        'most_popular_barber': popular_barber.get_full_name() if popular_barber else None,
    }
...
def get_barber_performance(barber_id: str) -> Optional[Dict]:
    ...
    if total_appointments > 0:
        completion_rate = (Decimal(completed_appointments) / Decimal(total_appointments)) * 100
...
def get_revenue_analytics(period: str = 'daily', days: int = 30) -> List[Dict]:
    ...
    if period == 'daily':
        ...
        while current_date <= end_date:
            ...
            average_ticket = revenue / appointments_count if appointments_count > 0 else Decimal('0.00')
...
def get_service_popularity(barbershop_id: Optional[str] = None) -> List[Dict]:
    ...
    services = services.annotate(
        appointment_count=Count('appointments'),
        total_revenue=Sum('appointments__payment__amount', 
    ...
    for service in services:
        ...
        if total_appointments > 0:
            booking_percentage = (Decimal(service.appointment_count) / Decimal(total_appointments)) * 100
        ...
        popularity_data.append({
            ...
            'average_rating': round(service.avg_rating, 2) if service.avg_rating else Decimal('0.00'),
...
def get_customer_insights(barbershop_id: Optional[str] = None) -> Dict:
    ...
    new_customers_this_month = customers.filter(
        first_visit__gte=start_of_month
    ).count()
    
    # Clientes que retornaram
    returning_customers = customers.filter(visit_count__gt=1).count()
    
    # Média de agendamentos por cliente
    total_appointments = sum(customer.visit_count for customer in customers)
    avg_appointments = Decimal(total_appointments) / Decimal(total_customers) if total_customers > 0 else Decimal('0.00')
    
    # Taxa de retenção (clientes com mais de 1 visita)
    retention_rate = (Decimal(returning_customers) / Decimal(total_customers)) * 100 if total_customers > 0 else Decimal('0.00')
    
    # Cliente mais frequente
    frequent_customer = customers.order_by('-visit_count').first()
    ...
    return {
        ...
        'most_frequent_customer': frequent_customer.customer.get_full_name() if frequent_customer else None,
    }